...
goog.scope(() => {
  ...
  goog.module.ModuleManager.prototype.loadModules_ = function(ids, opt_isRetry, opt_forceReload) {
    ...
    var loadFn = goog.bind(this.getLoader().loadModules, goog.asserts.assert(this.getLoader()), goog.array.clone(idsToLoadImmediately), goog.asserts.assert(this.moduleInfoMap), {extraEdges:this.extraEdges_, forceReload:!!opt_forceReload, onError:status => {
      return this.handleLoadError_(this.requestedLoadingModuleIds_, idsToLoadImmediately, status != null ? status : undefined);
  ...
  goog.module.ModuleManager.prototype.isModuleLoading = function(id) {
    ...
    var i = 0;
    for (; i < this.requestedModuleIdsQueue_.length; i++) {
  ...
  goog.module.ModuleManager.prototype.dispatchModuleLoadFailed_ = function(cause) {
    ...
    i = 0;
    for (; i < failedIds.length; i++) {
      goog.array.insert(idsToCancel, failedIds[i]);
    }
    i = 0;
    for (; i < idsToCancel.length; i++) {
      var j = 0;
      for (; j < this.requestedModuleIdsQueue_.length; j++) {
  ...
  goog.module.ModuleManager.prototype.executeCallbacks_ = function(type) {
    ...
    var i = 0;
    for (; callbacks && i < callbacks.length; i++) {
      callbacks[i](type);
    }
  };
  goog.module.ModuleManager.prototype.visitDependencies_ = function(ids, visitorFn, visitConditionFn = () => {
    return true;
  }, visited = {}) {
    for (const id of ids) {
